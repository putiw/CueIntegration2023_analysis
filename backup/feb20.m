...
%     tmp=mean(tmp,3);
    ...
    tmp = zeros(176,size(regressed_data,2),5);
    for iT = 4:8
        tmp(:,:,iT-3)= zscore(regressed_data(find(dmS)-1+iT,:));
    end
    tmp=mean(tmp,3);
    ...
    %%
%     global_signal = mean(tmp, 2);
%     X = [ones(size(global_signal)), global_signal];
%     X(isnan(X))=0;
%     coefficients = X \ tmp;
%     tmp = tmp - X * coefficients;
    %%
    lb = label(:,3);
